...
EModuleRenderer3D::EModuleRenderer3D(bool start_enabled) : EModule(start_enabled)
{
	vsync = false;
	screen_width = 1024;
	screen_height = 768;
...
// Destructor
EModuleRenderer3D::~EModuleRenderer3D()
{}

// Called before render is available
bool EModuleRenderer3D::Init()
{
	ELOG("Creating 3D Renderer context");
	bool ret = true;

	if (targetWindow == NULL)
	{
		ELOG("Target window has not been set. Try initializing the variable with 'SetTargetWindow()'");
		ret = false;
	}
	...
	if (context == NULL)
	{
		ELOG("OpenGL context could not be created! SDL_Error: %s\n", SDL_GetError());
		ret = false;
	}
	...
	if (ret == true)
	{
		...
		if (vsync && SDL_GL_SetSwapInterval(1) < 0)
			ELOG("Warning: Unable to set VSync! SDL Error: %s\n", SDL_GetError());
		...
		{
			ELOG("Error initializing OpenGL! %s\n", gluErrorString(error));
			ret = false;
		}
...
engine_status EModuleRenderer3D::PreUpdate()
{
...
engine_status EModuleRenderer3D::Update()
{
...
engine_status EModuleRenderer3D::PostUpdate()
{
...
bool EModuleRenderer3D::CleanUp()
{
	ELOG("Destroying 3D Renderer");
...
void EModuleRenderer3D::OnResize(int width, int height)
{
...
void EModuleRenderer3D::DrawGrid(int size, int step, bool xzAxis, bool xyAxis, bool zyAxis)
{
	glLineWidth(1.0);
